...
const MegMemories = () => {
  ...
  const megMemories =
    memories.memories && memories.memories.filter(memory => memory.child === 'meg');

  const getMemories = useCallback(async () => {
    try {
      globalLoading.setIsLoading();
      const response = await memoryApi.getOneChildsMemories('meg');
  ...
  return <MemoriesContainer data={megMemories} />;
};

export default MegMemories;
